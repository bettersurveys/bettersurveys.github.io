let e,a,t,i=0;function n(){return i=(i+1)%Number.MAX_SAFE_INTEGER}async function s(e,a){let t=await fetch(e,a);if(t.ok)return t;throw t}function l(e){let a=document.createElement("template");return a.innerHTML=e.trim(),a.content.firstChild}class r{constructor(){this.event=new EventTarget,this.aux=new Event("e")}addListener(e,a){let t=this.event;e.myCallbacks?e.myCallbacks.push(a):e.myCallbacks=[a];let i=[new WeakRef(e),new WeakRef(a)];t.addEventListener("e",function e(){let[a,n]=i.map(e=>e.deref());a&&n&&(document.contains(a)?n():t.removeEventListener("e",e))})}addGlobalListener(e){this.event.addEventListener("e",e)}dispatch(){this.event.dispatchEvent(this.aux)}}class o{constructor(e){this.variable=e,this.event=new r}get value(){return this.variable}set value(e){this.variable=e,this.event.dispatch()}subscribe(e,a){this.event.addListener(e,a)}addGlobalListener(e){this.event.addGlobalListener(e)}}const c=new Worker("assets/webworker.js",{type:"module"}),d=new Map;async function p(){new Promise(e=>setTimeout(e))}async function u(){return new Promise(async e=>{await p(),d.size?c.addEventListener("message",async function a(){await p(),d.size||(c.removeEventListener("message",a),e())}):e()})}async function m(e,a,t){a&&await u();let i=n();return new Promise((a,n)=>{d.set(i,{onSuccess:a,onError:n}),c.postMessage({python:e,id:i,...t})})}async function h(e,a){return m(`
	...
	</main>`)}async function eJ(e){let a=l(`<section class="vars-table">
		<article>${await R("activeVars")}</article>
	</section>`);for(let t of e)a.append(l(`<article>${t}</article>`));return a}async function eX(e){let a=new Set([...e.options].map(e=>e.value));for(let t of eB)if(t=await R(t),a.has(t)){e.value=t;return}}async function eK(){let e=l(`<main class="main-content">
		<header>
			<h2>${await R("weightTitle")}</h2>
			<h3>${W.value}</h3>
		</header>
	...
	</main>`);function t(a){return e.querySelector(a)}let[i,n,s]=["#method",".extra h4",".extra p"].map(t),r=["#orig-weights",'[for="orig-weights"]'].map(t),o=["#pop-size",'[for="pop-size"]'].map(t),c=t(".vars-table + a"),d=t("#new-var-name"),p=t("button"),u=await a;for(let a of(r[0].append(...(await eg.filtered(e=>e.isNpNumeric)).map(e=>l(`<option>${e.name}</option>`))),u&&(o[0].valueAsNumber=u),c.addEventListener("click",()=>z.value="data"),p.onclick=()=>i.reportValidity(),[new eG,new eI,new eI(!0),new eI(!1,!0),new eI(!0,!0)])){let t=await a.variables;if(t.length){let[u,m,h]=await Promise.all([a.name,a.title,a.description]);i.append(l(`<option>${u}</option>`)),i.addEventListener("change",async()=>{i.value==u&&(n.innerHTML=m,s.innerHTML=h,function(e,a,t){e.forEach(e=>e.hidden=!t),e[0].onchange=()=>a.forEach(a=>a.hidden=t&&!!e[0].value),e[0].onchange()}(r,o,a.acceptsOrig),e.querySelector(".vars-table").replaceWith(await eJ(t.map(e=>e.name))),c.hidden=!1,p.onclick=async()=>{if(d.reportValidity()&&(o[0].hidden||o[0].reportValidity())){let t=await J(await R("weightLoading"));e.append(t),t.showModal(),a.estimate(d.value,o[0].valueAsNumber,r[0].value).then(()=>{D.value=d.value,z.value="data"}).catch(async e=>{console.error(e),U(await a.errorMsg),t.remove()})}})})}}return await eX(i),i.dispatchEvent(new Event("change")),e}async function eQ(){return(await eg.variables()).some(e=>e.selected.value)?eK():eU()}async function eY(){let e=await ec();return e.querySelector("main").replaceWith(await eQ()),e}function eZ(e,a){return l(`<option${a?` value="${a}"`:""}>${e}</option>`)}async function e0(e,a,t,i=!1){let n=l(`<label>
		<span>${a}</span>
		<select id="${e}"${i?" disabled":""}>
			<option value="">${await R("uniformWeights")}</option>
		</select>
	</label>`);return n.querySelector("select").append(...t.map(e=>eZ(e))),n}async function e1(e,a,t,i=!1){let n=l(`<label>
		<span>${a}</span>
	...
	</label>`);return t&&n.querySelector("select").append(eZ(await R("linearMatching"),"linear"),eZ(await R("boostedMatching"),"boosting")),n}async function e2(e,a){let t=await a;return e.replaceWith(t),t.querySelector("select")}async function e4(e){let a=l(`<section class="selectors">
		<section class="row">
			<label>
	...
	</main>`);async function a(...t){let i=await J(await R("estimationLoading"));e.append(i),i.showModal(),await e5(...t).then(a=>e.querySelector("main").replaceWith(a)).catch(async a=>{console.error(a),U(await R("estimationError")),e.querySelector("#estimate").disabled=!1}),i.remove()}return e.querySelector(".selectors").replaceWith(await e4(a)),e}async function e7(){let e=await ec();return e.querySelector("main").replaceWith(await e6()),e}async function e8(){let e;scrollTo(0,0);let a=t=n();if("load"==z.value)e=eh();else if("data"==z.value)e=eA();else if("weight"==z.value)e=eY();else if("estimation"==z.value)e=e7();else throw Error(`Invalid screen: ${z.value}`);e=await e,a==t&&document.querySelector(".main-container > main").replaceWith(e)}z.addGlobalListener(e8),e8();