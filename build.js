let e,a,t,i,n=0;function s(){return n=(n+1)%Number.MAX_SAFE_INTEGER}async function l(e,a){let t=await fetch(e,a);if(t.ok)return t;throw t}function r(e){let a=document.createElement("template");return a.innerHTML=e.trim(),a.content.firstChild}class o{constructor(){this.event=new EventTarget,this.aux=new Event("e")}addListener(e,a){let t=this.event;e.myCallbacks?e.myCallbacks.push(a):e.myCallbacks=[a];let i=[new WeakRef(e),new WeakRef(a)];t.addEventListener("e",function e(){let[a,n]=i.map(e=>e.deref());a&&n&&(document.contains(a)?n():t.removeEventListener("e",e))})}addGlobalListener(e){this.event.addEventListener("e",e)}dispatch(){this.event.dispatchEvent(this.aux)}}class c{constructor(e){this.variable=e,this.event=new o}get value(){return this.variable}set value(e){this.variable=e,this.event.dispatch()}subscribe(e,a){this.event.addListener(e,a)}addGlobalListener(e){this.event.addGlobalListener(e)}}const d=new Worker("assets/webworker.js",{type:"module"}),u=new Map;async function p(){new Promise(e=>setTimeout(e))}async function m(){return new Promise(async e=>{await p(),u.size?d.addEventListener("message",async function a(){await p(),u.size||(d.removeEventListener("message",a),e())}):e()})}async function h(e,a,t){a&&await m();let i=s();return new Promise((a,n)=>{u.set(i,{onSuccess:a,onError:n}),d.postMessage({python:e,id:i,...t})})}async function w(e,a){return h(`
	...
	`,a)}function v(e){return`await (await pyfetch("${e}")).memoryview()`}function f(){let e=location.pathname;return e.endsWith("/")?e:e+"/"}async function b(e,a,t){return h(`
		from pyodide.http import pyfetch
		with open("${a}", "wb") as my_file:
			my_file.write(${v(e)})
	`,t)}d.onmessage=function(e){let{id:a,...t}=e.data,{onSuccess:i,onError:n}=u.get(a);u.delete(a),Object.hasOwn(t,"error")?n(t.error):i(t.result)};const g=(async()=>{await b(`${f()}assets/utils.py`,"utils.py"),await h("import utils")})(),y=w("openpyxl");async function $(e,a){await y,await h(`
		from pyodide.http import pyfetch
	...
	`)}async function S(e,a){let t=URL.createObjectURL(e);await $(t,a),URL.revokeObjectURL(t)}async function E(e,a){var t,i;let n;await y,await g,t=await h(`utils.to_excel(${e})`),i=`${a}.xlsx`,(n=document.createElement("a")).href=t,n.download=i,document.body.append(n),n.click(),URL.revokeObjectURL(t),n.remove()}async function L(e,a){return S(e,a)}async function k(e,a,t){if("Excel"==t)return E(e,a);throw Error(`Invalid extension: ${t}`)}async function q(e,a){return h(`
		${a} = ${e}
		del ${e}
	`)}async function T(e){return h(`del ${e}`)}async function _(e){return h(`${e}.columns.tolist()`)}async function x(e){return await g,h(`utils.num_vars(${e})`)}function N(e,a){return e&&e!=a?`, "${e}"`:""}async function P(e,a,t){return await g,h(`utils.harmonized_variables(${e}, ${a}${N(t)})`)}async function M(e,a,t,i){return await g,h(`utils.${a?"numeric":"categories"}_details("${e}", ${t}${N(i,e)})`)}async function W(e,a,t,i){await g;let n=await h(`utils.${a?"numeric":"categories"}_inferred_totals("${e}", ${t}${N(i)})`);return a?n:new Map(n)}async function D(e,a){return await g,await h(`utils.pop_total(${e}${N(a)})`)}y.then(()=>h("import openpyxl"));const z=new c("load"),F={value:!1},H=new c,O=new c;class A{constructor(e){this.targetName=e,this.event=new o}subscribe(e,a){this.event.addListener(e,a)}cancel(e){if(this[e]){let a=this[e].id;this[e].loadPromise.then(()=>T(a)),this[e]=null,this.event.dispatch()}}loadTemp(e){this.cancel("temp");let a={id:`temp${s()}`,filename:e.name};return a.loadPromise=L(e,a.id),this.temp=a,this.event.dispatch(),a.loadPromise.then(()=>{a.id==this.temp?.id&&(a.loaded=!0,this.event.dispatch())}).catch(e=>{if(a.id==this.temp?.id)throw console.error(e),this.temp=null,this.event.dispatch(),e})}confirmTemp(){this.cancel("final"),this.final=this.temp,this.temp=null,this.event.dispatch()}confirmFinal(){let a=this.final;return this.final=null,this.event.dispatch(),q(a.id,this.targetName).then(()=>{let t=a.filename.replace(/\.[^.]+$/,"");if("np_sample"==this.targetName)H.value=t;else if("p_sample"==this.targetName)e=a.weights,O.value=t;else throw Error(`Invalid name: ${this.targetName}`)})}}const C=new A("np_sample"),V=new A("p_sample");for(let e of[C,V])z.addGlobalListener(()=>e.cancel("final"));O.addGlobalListener(()=>{a=O.value&&D("p_sample",e)});const B=l("assets/texts.json").then(e=>e.json());async function j(e){return B.then(a=>a[e])}const R={maximumFractionDigits:2,minimumFractionDigits:2,maximumSignificantDigits:3,minimumSignificantDigits:3,roundingPriority:"morePrecision"},G=Intl.NumberFormat(void 0,R);R.style="percent";const I=Intl.NumberFormat(void 0,R);function J(e){let a=!1;return()=>{a||(a=!0,setTimeout(()=>{a=!1,e()}))}}function U(e){for(let a of e.querySelectorAll("dialog"))a.querySelector(".close-button").onclick=()=>a.close()}async function K(e){let a=r(`<dialog class="modal error">
		...
		<nav class="tabs"></nav>
		<main></main>
	</main>`);return e.querySelector(".tabs").replaceWith(await ed(z.value,H.value)),H.value&&e.querySelector(".main-title").append(await ep()),e}async function eh(e){let a=r(`<label class="radio-label">
		<input type="radio" name="load-type">
		...
		</section>`}async function ef(e=1){let a=r(`<main class="main-content">
		<header>
		...
		</label>`));let t=await eE.filtered(a=>a["right"==e?"inP":"inNp"]),[i,n]=a.querySelectorAll("input");i.addEventListener("change",()=>{t.forEach(e=>{e.expanded.value!=i.checked&&(e.expanded.value=i.checked)})}),n?.addEventListener("change",()=>{t.forEach(e=>{e.selectable.value&&e.selected.value!=n.checked&&(e.selected.value=n.checked)})});let s=J(()=>i.checked=t.every(e=>e.expanded.value)),l=J(()=>{let e=t.filter(e=>e.selectable.value);n.checked=e.length&&e.every(e=>e.selected.value)});for(let e of t)e.expanded.subscribe(a,s),n&&(e.selected.subscribe(a,l),e.selectable.subscribe(a,l));return s(),n&&l(),a}async function eF(e){let a=r('<article class="variable-status"></article>');return e.isHarmonized?a.innerHTML='<img width="24" height="24" src="images/check-circle.svg"/>':e.harmonReason?a.innerHTML=`<span class="lead left"><img src="images/lead.svg"/><span class="tooltip">${await j(e.harmonReason+"Reason")}</span></span><img src="images/check-yellow.svg"/>`:a.innerHTML='<img src="images/block.svg"/>',a}async function eH(e,a){let t,i,n=r(`<article class="variable view ${"single"!=a?a:""} ${e.isHarmonized?"harmonized":""}">
		<label class="expand-arrow icon">
			<input type="checkbox" hidden/>
			<img src="images/arrow-right.svg" data-inline/>
			<img src="images/arrow-down.svg" data-inline/>
		...
		<div class="border"></div>
		<span>${e.name}</span>
	</article>`),s=n.querySelector("input");return s.checked=e.expanded.value,s.addEventListener("change",()=>e.expanded.value=s.checked),e.expanded.subscribe(n,()=>{s.checked=e.expanded.value,e_(n,s,e,a)}),"right"!=a&&n.querySelector(".border").after(((i=(t=r(`<label class="checkbox icon">
		<input type="checkbox" hidden/>
		<img src="images/check.svg"/>
	</label>`)).querySelector("input")).checked=e.selected.value,i.disabled=!e.selectable.value,i.addEventListener("change",()=>e.selected.value=i.checked),e.selected.subscribe(t,()=>i.checked=e.selected.value),e.selectable.subscribe(t,()=>i.disabled=!e.selectable.value),t)),"right"==a?n.append(await eF(e)):n.append(...await eD(e)),e_(n,s,e,a),n}function eO(){return r('<div class="border"></div>')}function eA(e,a,t){e.expanded.subscribe(t,()=>{a.expanded.value!=e.expanded.value&&(a.expanded.value=e.expanded.value)})}async function eC(){let e=[];if(eE.areDual){e.push(await ez("left"),eO(),await ez("right"));let[a,t,i]=await Promise.all([eE.common(),eE.npOnly(),eE.pOnly()]),n=Math.max(t.length,i.length);for(let t of a)e.push(await eH(t,"left"),eO(),await eH(t,"right"));for(let a=0;a<n;a++){let n,s;t[a]&&(n=await eH(t[a],"left"),e.push(n)),e.push(eO()),i[a]&&(s=await eH(i[a],"right"),e.push(s)),t[a]&&i[a]&&(eA(t[a],i[a],n),eA(i[a],t[a],s))}}else{e.push(await ez("single"));let a=(await eE.variables()).map(e=>eH(e,"single"));e.push(...await Promise.all(a))}return e}async function eV(){let e;eE.variables().catch(async e=>{console.error(e),K(await j("loadFileFinalError")),z.value="load"});let a=r('<main class="main-content"></main>');if(eE.areDual){if(e=r('<section class="dual-container"></section>'),F.value){let e=(await j("loadedFilesSubtitle")).replace("$nVars",(await eE.harmonized()).length);a.append(Q(await j("loadedFilesTitle"),e))}}else e=r('<section class="dual-container single"></section>');return e.append(...await eo()),e.append(...await eC()),a.append(e),U(a),F.value=!1,eE.subscribe(a,async()=>a.replaceWith(await eV())),a}async function eB(){let e=await em();return e.querySelector("main").replaceWith(await eV()),e}async function ej(e){let a=await eE.harmonized(),t=r(`<section class="selectors">
		<section class="row">
			<label>
	...
	</main>`);function t(a){return e.querySelector(a)}let[i,n,s]=["#method",".extra h4",".extra p"].map(t),l=["#orig-weights",'[for="orig-weights"]'].map(t),o=["#pop-size",'[for="pop-size"]'].map(t),c=t(".vars-table + a"),d=t("#new-var-name"),u=t("button"),p=await a;for(let a of(l[0].append(...(await eE.filtered(e=>e.isNpNumeric)).map(e=>r(`<option>${e.name}</option>`))),p&&(o[0].placeholder=o[0].placeholder.replace(/\d+/,Math.round(p))),c.addEventListener("click",()=>z.value="data"),u.onclick=()=>i.reportValidity(),[new e9,new ae,new ae(!0),new ae(!1,!0),new ae(!0,!0)])){let t=await a.variables;if(t.length){let[p,m,h]=await Promise.all([a.name,a.title,a.description]);i.append(r(`<option>${p}</option>`)),i.addEventListener("change",async()=>{i.value==p&&(n.innerHTML=m,s.innerHTML=h,function(e,a,t){e.forEach(e=>e.hidden=!t),e[0].onchange=()=>a.forEach(a=>a.hidden=t&&!!e[0].value),e[0].onchange()}(l,o,a.acceptsOrig),e.querySelector(".vars-table").replaceWith(await ai(t.map(e=>e.name))),c.hidden=!1,u.onclick=async()=>{if(d.reportValidity()&&(o[0].hidden||o[0].reportValidity())){let t=await X(await j("weightLoading"));e.append(t),t.showModal(),a.estimate(d.value,o[0].valueAsNumber,l[0].value).then(()=>{F.value=d.value,z.value="eval"}).catch(async e=>{console.error(e),K(await a.errorMsg),t.remove()})}})})}}return await an(i),i.dispatchEvent(new Event("change")),e}async function al(){return(await eE.variables()).some(e=>e.selected.value)?as():at()}async function ar(){let e=await em();return e.querySelector("main").replaceWith(await al()),e}async function ao(e,a,t,i=!1){let n=r(`<label>
		<span>${a}</span>
	...
	</label>`);return n.querySelector("select").append(...t.map(e=>Y(e))),n}const af=["advancedPsa","advancedPsaBoost","advancedKw","advancedKwBoost","advancedLinearMatching","advancedBoostedMatching","advancedLinearTraining","advancedBoostedTraining"];async function ab(e,a){let t=await Promise.all(af.map(async e=>Y(await j(e),e)));e.append(...t),a.addEventListener("change",()=>{if(a.value)for(let a of t)e.value==a.value&&(e.value=""),a.hidden=!0;else t.forEach(e=>{e.hidden=!1})})}async function ag(e,a,t,i,n=!1){let s=r(`<label>
		<span>${a}</span>
	...
	</label>`);if(t){let e=s.querySelector("select");e.append(Y(await j("linearMatching"),"linear"),Y(await j("boostedMatching"),"boosting")),ab(e,i)}return s}async function ay(e,a){let t=await a;return e.replaceWith(t),t.querySelector("select")}async function a$(e){let a=r(`<section class="selectors">
		<section class="row">
			<label>
	...
	</main>`),[p,m]=u.querySelectorAll("article");p.append(Z(...await aS(a,o,c)));let h=await j("mainEstimation");return n&&(h=[h,await j("altEstimation")]),n?eX(a,h,o,c,m.lastElementChild):eK(a,h,o,m.lastElementChild),u}async function aL(e){return e.includes("different number of classes")||e.includes("minimum number of groups")?j("lowDataError"):j("estimationError")}async function ak(){let e=r(`<main class="main-content">
		<header>
	...
	</main>`);async function a(...t){let i=await X(await j("estimationLoading"));e.append(i),i.showModal(),await aE(...t).then(a=>e.querySelector("main").replaceWith(a)).catch(async a=>{console.error(a),K(await aL(a)),e.querySelector("#estimate").disabled=!1}),i.remove()}return e.querySelector(".selectors").replaceWith(await a$(a)),e}async function aq(){let e=await em();return e.querySelector("main").replaceWith(await ak()),e}const aT=import("https://cdn.jsdelivr.net/npm/marked/lib/marked.esm.min.js").then(e=>e.marked),a_=document.querySelector("section.help-section"),ax=a_.querySelector("article");function aN(){a_.classList.toggle("active")}async function aP(){ax.innerHTML="";let e=t=s(),a=l(`help/${z.value}.md`).then(e=>e.text()).catch(e=>(console.error(e),"# Error")),i='<button class="close-button"><img src="images/close.svg" data-inline/></button>'+(await aT).parse(await a);e==t&&(ax.innerHTML=i,ax.querySelector("button").addEventListener("click",aN),ax.querySelectorAll("a").forEach(e=>{e.target="_blank",e.rel="noopener"}))}async function aM(){let e;scrollTo(0,0);let a=i=s();if("load"==z.value)e=eb();else if("data"==z.value)e=eB();else if("bias"==z.value)e=e2();else if("weight"==z.value)e=ar();else if("eval"==z.value)e=aw();else if("estimation"==z.value)e=aq();else throw Error(`Invalid screen: ${z.value}`);e=await e,a==i&&document.querySelector(".main-container > main").replaceWith(e)}a_.querySelector("button").addEventListener("click",aN),a_.querySelector(".help-background").addEventListener("click",aN),z.addGlobalListener(aP),aP(),z.addGlobalListener(aM),aM();